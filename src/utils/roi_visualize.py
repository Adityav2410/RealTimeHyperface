...
roi_pool_1 = model_classifier_only.get_layer('roi_pool_1').output
roi_pool_2 = model_classifier_only.get_layer('roi_pool2').output
roi_pool_4 = model_classifier_only.get_layer('roi_pool_4').output
model_roi_pool = Model([b1_inp, b2_inp, b4_inp, roi_input], [roi_pool_1, roi_pool_2, roi_pool_4] )
...
try:
	...
	model_rpn.load_weights('./model_hyper.hdf5', by_name=True)   #TODO: load RPN weights
...
model_roi_pool.compile(optimizer='sgd',loss='mse')
...
# img_path = '../viz_data/viz_images'
# img_path = '../viz_data/viz_images'
img_path = '../../Emotion_Hackathon/data/Hackathon_Data/'
img_save_path = '../viz_data/viz_annotated_images/'
roi_save_path = '../roi_dump/'
...
for img_name in files:
	try:
		...
		R = roi_helpers.rpn_to_roi(Y1, Y2, C, K.image_dim_ordering(), use_regr=True, overlap_thresh=0.7, max_boxes=1)
		...
		for jk in range(R.shape[0]//C.num_rois + 1):
			...
			# bp()
			[roi_pool_1, roi_pool_2, roi_pool_4] = model_roi_pool.predict([feat1, feat2, feat4 , ROIs])

			if len(roi_dict.keys()) == 0:
				roi_dict['roi_pool_1'] = roi_pool_1
				roi_dict['roi_pool_2'] = roi_pool_2
				roi_dict['roi_pool_4'] = roi_pool_4
			else:
				roi_dict['roi_pool_1'] = np.concatenate([roi_dict['roi_pool_1'] , roi_pool_1],axis = 1)
				roi_dict['roi_pool_2'] = np.concatenate([roi_dict['roi_pool_2'] , roi_pool_2],axis = 1)
				roi_dict['roi_pool_4'] = np.concatenate([roi_dict['roi_pool_4'] , roi_pool_4],axis = 1)
		with open(os.path.join(roi_save_path, os.path.splitext(img_name)[0]+'.pkl'), 'wb') as handle:
