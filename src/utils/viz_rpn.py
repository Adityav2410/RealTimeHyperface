...
for img_name in files:
	try:
		...
		plt.imshow(img)
		plt.savefig( img_save_path + base + '_' + str(0) + '.jpeg')
		for i in range(3):
			img_prob = cv2.resize(Y1[0,:,:,i], (new_width, new_height), interpolation=cv2.INTER_CUBIC)
			# img_prob = np.expand_dims(img_prob, axis=2)
			plt.imshow(img_prob)
			plt.savefig( img_save_path  + base + '_' + str(i+1) + '.jpeg' )

			# img_feature_map = img_scaled * img_prob
			# img_feature_map = img_feature_map.astype(int)
			# img_save = Image.fromarray(img_prob, 'RGB')
			# img_save.save( img_save_path + str(i) + '_' +img_name )
			# bp()
...
# 			textLabel2 = '{}:     {}'.format( new_genders[jk], new_probs[jk] )
# 			#all_dets.append((key,100*new_probs[jk]))

# 			(retval,baseLine) = cv2.getTextSize(textLabel,cv2.FONT_HERSHEY_COMPLEX,1,1)
# 			(retval2,baseLine2) = cv2.getTextSize(textLabel2,cv2.FONT_HERSHEY_COMPLEX,1,1)
# 			textOrg = (x1, y1-0)
# 			textOrg2 = ( x1, y2 )
...
# 			cv2.putText(img_scaled, textLabel2, textOrg2, cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 0), 1)
...
# 		# cv2.waitKey(0)
# 		cv2.imwrite( os.path.join(img_save_path,img_name) ,img_scaled)
# 		filename = 'image71306.jpg'
		
# 		orient = data_annotation[filename]['face'].values()[0]['orientation']#['orientation']
# 		orient_2 = (180/3.14) * np.array(orient) 
# 		orient_2.astype('int')
# 		print "Predicted orientation", textLabel
# 		print "True orientation", textLabel
# 	except:
