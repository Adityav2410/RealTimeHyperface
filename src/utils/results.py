...
# img_save_path = '../viz_data/viz_annotated_images_2'
...
# bp()
df = pd.DataFrame(columns=['filename','iou','score','pred_gender','true_gender','pred_orient','true_orient'])
...
for img_data in train_imgs:
	...
	imgReadStart = time.time()
	bp()

	filepath = os.path.join(img_path,img_data['filepath'])
	img = cv2.imread(filepath)
	print("Processing file: ", img_data['filepath'])
	X = format_img(img, C)

	print("Time to read and process image: ", time.time()- imgReadStart)
	...
	if( len(bboxes) == 0):
		...
		bboxes_rpn.append([5,6,7,8])
		probs.append(0.01)
		genders.append('f')
		poses.append(np.zeros(3))
		vizs.append(np.zeros(21))
	...
	if displayImages:
		...
		cv2.waitKey(0)
		cv2.imwrite( os.path.join(img_save_path,img_data['filepath']) ,img_scaled)
