...
classifier_regr_only  = nn.classifier_only_regr(b1_feat, b2_feat, b4_feat, roi_input, C.num_rois, nb_classes=len(classes_count), trainable=True)
# classifier = [face_out,pose_out, gender_out, viz_out, landmark_out]

model_rpn = Model(base_model.input, rpn[:2])
model_classifier = Model([img_input, roi_input], classifier_regr_only )
...
#################################### Make trainable false for last result layers ############################################################

# model_classifier.summary()
# turn_off_index_list = []
# layerNameList = ['fc_detection', 'fc_landmarks', 'fc_visibility','fc_pose','fc_gender']

# for index in range(len(turn_off_index_list)):
# 	model_classifier.get_layer(layerNameList[index]).trainable = False
...
model_all = Model([img_input, roi_input], rpn[:2] + classifier_regr_only)
...
while True:
	try:
		...
		#loss_class = model_classifier.train_on_batch([X, X2[:, sel_samples, :]], [face_true,pose_true,gender_true,viz_true,land_true] )
		loss_class = model_classifier.train_on_batch([X, X2[:, sel_samples, :]], [face_true,regr_true] )
		...
		losses[iter_num, 7] = loss_class[2]
		...
		if iter_num == epoch_length:
			...
			loss_class_pose 	= 	0#np.mean(losses[:, 3])
			loss_class_gender	=	0#np.mean(losses[:, 4])
			loss_class_viz		=	0#np.mean(losses[:, 5])
			loss_class_landmark = 	0#np.mean(losses[:, 6])
			...
			# 	fHandle.write('\nloss_rpn_cls,{},loss_rpn_regr,{},loss_class_cls,{},elapsed_time,{}'.format(loss_rpn_cls, loss_rpn_regr, loss_class_cls, time.time() - start_time))
			# else:
			...
			print('Detection:{:.4f}, Gender:{:.4f}, Pose:{:.4f}, Viz:{:.4f}, Landmark:{:.4f}, Regression:{:.4f}'.format(	loss_class_detect, loss_class_pose, loss_class_gender, loss_class_viz, loss_class_landmark, loss_class_regr ) )
			...
			fHandle.write('Detection:{:.4f}, Gender:{:.4f}, Pose:{:.4f}, Viz:{:.4f}, Landmark:{:.4f}, Regression:{:.4f}'.format(	loss_class_detect, loss_class_pose, loss_class_gender, loss_class_viz, loss_class_landmark, loss_class_regr ) )
			fHandle.write("\n")
			iter_num = 0
			start_time = time.time()
			epoch_num += 1
			if epoch_num == 1 or curr_loss < best_loss:
				...
				best_loss = curr_loss
				model_all.save_weights('model_regr_only.hdf5')
